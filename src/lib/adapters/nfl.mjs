// NFL adapter — thin wrapper around ESPN shared fetcher
...
export async function fetchScheduleWindow(team, startDateIso, endDateIso) {
  ...
  for (let i = 0; i <= days; i++) {
    ...
    const dayGames = await fetchEspnScoreboard("football", "nfl", dateKey);
...
export async function fetchLeagueScheduleToday() {
  ...
  const games = await fetchEspnScoreboard("football", "nfl", dateKey);
